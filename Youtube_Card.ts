...
async function YoutubeFirstLogin(Account) {
  ...
  await RPA.Logger.info('＊＊＊＊＊ログ出力します＊＊＊＊＊');
  const ThisPage = await RPA.WebBrowser.driver.getPageSource();
  await RPA.Logger.info(ThisPage);
  const GoogleLoginPW = await RPA.WebBrowser.findElementsById('Passwd');
  await RPA.Logger.info(GoogleLoginPW.length);
  if (GoogleLoginPW.length == 1) {
    await RPA.WebBrowser.sendKeys(GoogleLoginPW[0], [`${Account[2]}`]);
    const NextButton2 = await RPA.WebBrowser.findElementById('signIn');
    await RPA.WebBrowser.mouseClick(NextButton2);
  }
  if (GoogleLoginPW.length == 0) {
...
async function Youtube2ndLogin(Account) {
  ...
  if (GoogleLoginPW_1.length == 1) {
    await RPA.WebBrowser.sendKeys(GoogleLoginPW_1[0], [`${Account[2]}`]);
    const NextButton2 = await RPA.WebBrowser.findElementById('signIn');
    await RPA.WebBrowser.mouseClick(NextButton2);
  }
...
async function TitleSyousaiChange(CardData) {
  ...
  const SaveButton = await RPA.WebBrowser.findElementsByCSSSelector('#save');
